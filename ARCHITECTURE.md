...
- `smartgym_get_routine` per-exercise history was wrong in `smart-gym-mcp` v0.2.0 (found 2026-09-23): sets were grouped by row-creation date, with prescription rows mixed in, so repeated workouts collapsed into one. **Fixed in joeshoff/smart-gym-mcp@236af09 (Issue #7)**: history is grouped by workout through the `Z_11SETSDONE` join (explicit keys), and `smartgym_get_workout_detail(workout_pk)` is the Trainer's post-workout read. Behavior the Trainer must know: weights come back as raw `weight_kg` plus `weight` in SmartGym's display unit (lb); 0.0 means bodyweight *or* never entered; a set deleted or unchecked mid-workout still counts (as it does in the app); exercises with no logged sets are omitted (skipped vs. not planned can't be told apart); exercise order is the routine's current slot order. Open follow-up: a logged session with no `ZWORKOUT` row is invisible to the new tools (seen once, on a test routine); see ROADMAP.md.
- Completing an exercise in one routine can rewrite another routine's prescription for the same catalog exercise (observed 2026-09-23 by the Lead Engineer on Issue #7, from DB evidence). The 9/22 Lower A workout rewrote Lower B's standing leg curl prescription, and the 9/21 Upper A workout replaced Lower A's Pallof set 2. The app soft-deletes the old prescription row and writes a new one, so a routine's planned sets can change without anyone editing that routine. Consequences: re-read a routine's prescription before coaching from it; never assume it's what the Trainer last wrote; and "what's planned" for a shared exercise may reflect the most recent session in any routine. This also explains the 9/20 observation that history seemed to appear under every routine sharing an exercise. Not yet verified against the app UI.
- SmartGym does not store skipped exercises (Lead Engineer DB evidence, Issue #7, 2026-09-23). A workout's history record lists only the routine slots that have at least one logged set. The app's grayed-out "skipped" exercise appears to come from drawing the routine's **current** plan against the past workout, and routines get rewritten (see above), so what the app shows as skipped on an old workout may not match what was planned that day. Consequence for coaching: an exercise missing from a workout means "no sets logged" and nothing more. Whether it was skipped, swapped, or not planned can't be read from the data; ask Joe. Classified as an observation, not an Issue: no reliable data source exists to build on.
- **Write tools take kilograms only** (`SetSpec.weight_kg`), while reads since Issue #7 show `weight` in lb. Copying a read value into a write without converting (lb × 0.45359237) writes a ~2.2× load into a live prescription. Until the write-unit Issue ships, the Trainer converts explicitly and checks every dry-run plan for plausible kg values before applying.
