...
| P0 | Sessions without a `ZWORKOUT` row stay visible in history | Foundation | Low | Follow-up to #7: anchor history on `ZHISTORY` so a logged session never reads as "didn't train" |
| P0 | SmartGym write tools: explicit weight unit + plausibility guard | Foundation | Medium | Follow-up to #7: prevents lb values being written as kg into live prescriptions |
